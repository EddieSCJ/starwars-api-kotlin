...
class GlobalExceptionHandler @Autowired constructor(val messageSourceHelper: MessageSourceHelper) {
    ...
    @ApiResponse(
        ...
        content = [Content(
            ...
            schema = Schema(implementation = BaseExceptionResponseJson::class)
        )]
    )
    ...
    @ApiResponse(
        ...
        content = [Content(
            mediaType = MediaType.APPLICATION_JSON_VALUE,
            schema = Schema(implementation = BadRequestExceptionResponseJson::class)
        )]
    )
    fun handleBadRequest(exception: BadRequestError): ResponseEntity<BadRequestExceptionResponseJson> {
        val response = BadRequestExceptionResponseJson(
            HttpStatus.BAD_REQUEST.value(),
            messageSourceHelper.getApiErrorMessage("bad_request"),
            exception.errors
        )
        return ResponseEntity.status(response.httpStatusCode).body(response)
    }
